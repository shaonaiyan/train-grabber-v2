...
export class DebugPanel {
    ...
    isPaused = false;
    timeScale = 1.0;
    ignoreLoadLimit = false;
    showOverlays = false;
    constructor(scene, trainManager, grapple, director, enemyManager, itemFactory, allWorldItems, seed) {
        this.scene = scene;
        ...
        this.buildPanelUI();
        // Toggle with F1
        scene.input.keyboard?.on('keydown-F1', (evt) => {
            evt.preventDefault();
            this.toggle();
        });
    }
    ...
    buildPanelUI() {
        ...
        let btnX = 390;
        let btnY = 45;
        const createBtn = (label, onClick, color = 0x2980b9) => {
        ...
        createBtn('Toggle MaxLoad', () => {
            ...
            if (this.ignoreLoadLimit) {
                this.trainManager.load.currentMaxLoad = 9999;
            }
            else {
        ...
        phases.forEach((pName, idx) => {
            createBtn(pName, () => {
                ...
                this.scene.currentTime = targetTimes[idx];
        ...
        btnX = 390;
        btnY += 34;
        createBtn('+ Bandit', () => this.enemyManager.spawnBandit(), 0xc0392b);
        createBtn('+ Drone', () => this.enemyManager.spawnDrone(), 0xd35400);
        ...
        oppLetters.forEach((letter) => {
            createBtn(`Group ${letter}`, () => {
                const grp = this.director.groups[letter];
                if (grp) {
                    this.director.spawnWindowWithItems(`debug_${letter}_${Date.now()}`, letter, grp.name, grp.items, this.scene.time.now * 0.001, 4.0, this.allWorldItems);
                }
            }, 0xd35400);
        });
        // Force Win / Fail
    ...
    update(timeSec, phaseId, phaseName) {
        if (!this.isVisible)
            return;
