...
export class TrainCar {
    scene;
    type;
    carIndex; // 0 is Locomotive, 1 is Crane, 2 is Cargo, 3+ are Flat cars
    container;
    width;
    slots = [];
    baseCarX = 0;
    ...
    wheelSprites = [];
    wheelAngle = 0;
    onModuleDiscardCallback;
    onModuleHoverCallback;
    constructor(scene, type, carIndex, onDiscard, onHover) {
        this.scene = scene;
        this.type = type;
    ...
    buildCarVisuals() {
        ...
        // Wheels
        const wheelXPositions = this.type === 'locomotive'
            ? [-60, -20, 25, 65]
            : [-50, -20, 20, 50];
    ...
    update(time, speed, powerEfficiency, hasPowerShortage) {
        // Wheels rotation
        ...
        for (const slot of this.slots) {
            if (slot.installedModule && slot.container) {
                if (slot.installedModule.itemId === 'turret') {
                    const led = slot.container.getByName('powerLed');
                    if (led) {
                        if (hasPowerShortage) {
    ...
    aimTurrets(targetX, targetY) {
        for (const slot of this.slots) {
            if (slot.installedModule && slot.installedModule.itemId === 'turret' && slot.container) {
                const barrels = slot.container.getByName('turretBarrels');
                if (barrels) {
