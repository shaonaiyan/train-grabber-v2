...
export class GrappleVisual {
  ...
  private craneArmGraphics: Phaser.GameObjects.Graphics;
  private chainGraphics: Phaser.GameObjects.Graphics;
  public clawContainer: Phaser.GameObjects.Container;
  private clawGraphics: Phaser.GameObjects.Graphics;

  private boomLength: number = 55;
  ...
  public drawClaw(isClamping: boolean): void {
  ...
  public updateVisuals(
    ...
    const boomTipX =
      craneBaseX + Math.cos(angle) * (this.boomLength - this.recoilOffset);
      const boomTipY =
      craneBaseY + Math.sin(angle) * (this.boomLength - this.recoilOffset);
