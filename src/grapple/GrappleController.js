...
export class GrappleController {
    ...
    fire(targetX, targetY, availableItems) {
        if (this.fsm.isBusy())
            return false;
        const cranePos = this.trainManager.getCranePosition();
    ...
    update(delta, availableItems) {
        ...
        // Update visuals
        this.visual.updateVisuals(cranePos.x, cranePos.y, this.hookX, this.hookY, this.fsm.isIdle(), this.latchedItem !== null);
    }
    updateFlying(dt, availableItems, cranePos) {
        ...
        for (const item of availableItems) {
            if (item.isLatched || item.isDelivered || item.isDestroyed)
                continue;
            const dist = Phaser.Math.Distance.Between(this.hookX, this.hookY, item.container.x, item.container.y);
    ...
    updatePulling(dt, cranePos) {
        ...
        const pullSpeed = Phaser.Math.Clamp(balanceData.hook.basePullSpeed - weight * balanceData.hook.weightPullPenalty, balanceData.hook.minPullSpeed, balanceData.hook.basePullSpeed);
    ...
    checkHoldingResolution() {
        if (this.fsm.getState() !== 'REJECTED' || !this.latchedItem)
            return;
        const itemData = this.latchedItem.data;
        const fitsLoad = this.trainManager.canFitLoad(itemData);
        const hasSlot = this.trainManager.hasSlotFor(itemData);
