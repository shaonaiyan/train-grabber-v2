...
export class TelemetryManager {
    ...
    setupEventListeners() {
        ...
        bus.on('ITEM_SEEN', (d) => {
            if (this.itemStats[d.item])
                this.itemStats[d.item].seen++;
            this.recordEvent('ITEM_SEEN', d);
        });
        bus.on('GRAPPLE_FIRE', (d) => this.recordEvent('GRAPPLE_FIRE', d));
        bus.on('GRAPPLE_HIT', (d) => {
            if (this.itemStats[d.item])
                this.itemStats[d.item].attempted++;
            this.recordEvent('GRAPPLE_HIT', d);
        });
        bus.on('GRAPPLE_MISS', (d) => this.recordEvent('GRAPPLE_MISS', d));
        bus.on('ITEM_DELIVERED', (d) => {
            if (this.itemStats[d.item])
                this.itemStats[d.item].grabbed++;
        ...
        bus.on('ITEM_REJECTED', (d) => this.recordEvent('ITEM_REJECTED', d));
        bus.on('ITEM_DISCARDED', (d) => {
            if (this.itemStats[d.item])
                this.itemStats[d.item].discarded++;
            this.discards.push({
                time: d.time,
                item: d.item,
                reasonContext: {
        ...
        bus.on('CAR_ATTACHED', (d) => this.recordEvent('CAR_ATTACHED', d));
        bus.on('POWER_SHORTAGE_START', (d) => this.recordEvent('POWER_SHORTAGE_START', d));
        bus.on('POWER_SHORTAGE_END', (d) => this.recordEvent('POWER_SHORTAGE_END', d));
        bus.on('HEAVY_TRAIN_START', (d) => this.recordEvent('HEAVY_TRAIN_START', d));
        bus.on('HEAVY_TRAIN_END', (d) => this.recordEvent('HEAVY_TRAIN_END', d));
        bus.on('ENEMY_SPAWN', (d) => this.recordEvent('ENEMY_SPAWN', d));
        bus.on('ENEMY_KILLED', (d) => this.recordEvent('ENEMY_KILLED', d));
        bus.on('TRAIN_DAMAGE', (d) => this.recordEvent('TRAIN_DAMAGE', d));
        bus.on('FRIDGE_OPEN', (d) => this.recordEvent('FRIDGE_OPEN', d));
        bus.on('EGG_HATCH', (d) => this.recordEvent('EGG_HATCH', d));
        bus.on('PHASE_CHANGE', (d) => {
            this.currentPhaseName = d.name;
            this.recordEvent('PHASE_CHANGE', d);
        });
        bus.on('WINDOW_START', (d) => this.recordEvent('WINDOW_START', d));
        bus.on('WINDOW_END', (d) => {
            this.windows.push(d.record);
            for (const ign of d.record.itemsIgnored) {
                if (this.itemStats[ign])
                    this.itemStats[ign].ignored++;
            }
